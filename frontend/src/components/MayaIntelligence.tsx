...
export default function MayaIntelligence() {
  ...
  return (
    <motion.div
      ...
      className="bg-slate-900/50 border border-white/10 rounded-[1.5rem] p-4 backdrop-blur-2xl relative overflow-hidden group shadow-2xl"
    >
      {/* Background Glow */}
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-purple-500/20 to-emerald-500/20 rounded-xl flex items-center justify-center border border-white/10 shadow-lg group-hover:scale-110 transition-transform duration-500">
              <Brain className="w-4.5 h-4.5 text-purple-400" />
            </div>
            <div>
              <h3 className="text-[10px] font-black text-white uppercase tracking-[0.2em] mb-0.5">MAYA Intelligence</h3>
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3].map(i => (
              <Star key={i} className={`w-2.5 h-2.5 ${score > 80 ? 'text-amber-400 fill-amber-400' : 'text-slate-700'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="flex items-end gap-3 mb-4">
          ...
          >
            <div className="flex items-baseline gap-1">
              <span className="text-5xl font-black tracking-tighter text-white drop-shadow-[0_0_20px_rgba(168,85,247,0.4)]">
                {displayScore}
              </span>
              <span className="text-lg font-black text-slate-500 uppercase tracking-widest">%</span>
            </div>
          </motion.div>
