...
export default function Forecast() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const mlBalance = payload.find((p: any) => p.dataKey === 'ml_balance')?.value;

      return (
        <div className="bg-[#0f172a] border border-white/5 p-4 rounded-xl shadow-2xl backdrop-blur-xl">
          <p className="text-slate-500 font-bold mb-2 uppercase text-[10px] tracking-widest">{label}</p>

          <div className="space-y-3">
            <div>
              <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mb-0.5">Core Forecast</p>
              <p className={`text-base font-black ${isDeclining ? 'text-rose-500' : 'text-emerald-500'}`}>
            ...
            {mlBalance !== undefined && (
              <div>
                <p className="text-[9px] text-indigo-400 font-black uppercase tracking-widest mb-0.5">ML Regression</p>
                <p className="text-base font-black text-indigo-300">
                  ₹{mlBalance?.toLocaleString()}
                </p>
              </div>
            )}
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart
                data={data?.months.map((m, i) => ({
                  ...m,
                  ml_balance: data.ml_prediction && data.ml_prediction[i * 7] ? data.ml_prediction[i * 7] : null
                }))}
              ...
              >
                ...
                {/* Traditional Heuristic Forecast */}
                <Area
                  type="monotone"
                  dataKey="balance"
                  stroke={chartColor}
                ...
                {data?.ml_prediction && (
                  <Area
                    ...
                    name="ML Regression Intelligence"
                  />
                )}
              </AreaChart>
            </ResponsiveContainer>
